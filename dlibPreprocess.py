...
def hogsLandmarks(resizedCroppedImage, resizedCroppedLandmarks, folder):
    ...
    mask = grid_points_in_poly(shape=np.array(resizedCroppedImage).shape, verts= list(zip(resizedCroppedLandmarks[hull.vertices][:,1], resizedCroppedLandmarks[hull.vertices][:,0]))) # for some reason verts need to be flipped
